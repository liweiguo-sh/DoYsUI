...
function viewExtraButtonClick(jsonProp) {
    ...
    if (btnExtra.btnKey.equals("cancelSettlementBtn")) {
        ...
        for (var i = 0; i < btnExtra.dtbViewData.rowCount; i++) {
			...
			if (btnExtra.dtbViewData.rows[i].checked) {
                ...
                arrSelectedRows.push(btnExtra.dtbViewData.rows[i]["settlement_key"].value);
            }
        }

		if (!execut) {

            showMsg("包含未被确认的结算单，无法进行取消操作");
            return;
        }
        ...
        if (g.a.send("processType=g_mscm.settlement.Balance&actionType=cancelBalance", { keys: keys, viewKey: 'v_settlement_mscm' }, true)) {
			if (g.a.OK) {
    ...
    }else if (btnExtra.btnKey.equals("confirmSettlementBtn")) {
        ...
        if (g.a.send("processType=g_mscm.settlement.Balance&actionType=confirmBalance", { keys: keys, viewKey: 'v_settlement_mscm' }, true)) {
			if (g.a.OK) {
				var cReturn = g.a.cReturn;
				showMsg("结算单已确认成功！");
				gridView.refresh();
