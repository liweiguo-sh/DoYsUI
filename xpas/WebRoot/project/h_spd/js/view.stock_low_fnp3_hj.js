...
function viewExtraButtonClick(jsonProp) {
    ...
    if (btnExtra.btnKey.equals("createDist")) {
        ...
        for (var i = 0; i < btnExtra.dtbViewData.rowCount; i++) {
            ...
            if (dataRow.checked) {
                arrSelectedRows.push(dataRow["material_id"].value + "," + dataRow["qty_req"].value+","+dataRow["storage_id"].value);
            }
        }
        ...
        if (g.a.send("processType=h_spd.stock.Common&actionType=storageRepListToDistHJ", {
            ...
            if (g.a.OK) {
                ...
                var prop = {
                    url: g.appPath + "project/g_spd/html/storage/thd_dist_hj.html",
        ...
        if (g.a.send("processType=g_spd.storage.TapplyDistPrint&actionType=generatePrintSumDataHJ", { matrs: keys }, true)) {
            if (g.a.OK) {
                var cReturn = g.a.cReturn;
    ...
    }   else if (btnExtra.btnKey.equals("processSN")) {
